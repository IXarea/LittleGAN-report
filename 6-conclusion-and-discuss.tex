...
In this study, in order to obtain a smaller model, a smaller amount of training and running, and a model with good performance, we have carried out several experiments based on DCGAN\upcite{dcgan}. We finally proposed a facial image generation and adjustment model based on GAN\upcite{gan} and its training method. By adding a self-encoding network that shares some network layer parameters with GAN, the facial image adjustment in the image space with smaller information loss is realized. In the training aspect, we use the gradient penalty proposed in WGAN-GP\upcite{wgan-gp} and innovatively put forward The way of part training has basically achieved the research targets.
...
Therefore, the image resolution of our model output is also lower. In a later improvement, we will try to use a higher resolution tagged facial dataset, for example, CelebA-HQ\upcite{pix2pixhd}, to train with a higher performance server for a higher resolution model.
